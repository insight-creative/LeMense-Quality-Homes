...
jQuery(document).ready(function ($) {
  ...
  $('.cd-tab-filter li').on('click', function (event) {
    ...
    var selected_filter = $(event.target).data('type'); //check if user has clicked the placeholder item

    if ($(event.target).is(filter_tab_placeholder)) {
      filter_tab_placeholder_default_value == filter_tab_placeholder.text() ? filter_tab_placeholder.text(filter_tab_placeholder_text) : filter_tab_placeholder.text(filter_tab_placeholder_default_value);
      $('.cd-tab-filter').toggleClass('is-open'); //check if user has clicked a filter already selected 
      ...
      filter_tab_placeholder_text = $(event.target).text(); //add class selected to the selected filter item
  ...
  *************************************/
  ...
  $(".cd-filter-content input[type='search']").keyup(function () {
    ...
    delay(function () {
      inputText = $(".cd-filter-content input[type='search']").val().toLowerCase(); // Check to see if input field is empty

      if (inputText.length > 0) {
        $('.mix').each(function () {
          var $this = $(this); // add item to be filtered out if input text matches items inside the title   
...
var buttonFilter = {
  ...
  parseFilters: function parseFilters() {
    var self = this; // loop through each filter group and grap the active filter from each one.
  ...
  concatenate: function concatenate() {
    ...
    !self.outputString.length && (self.outputString = 'all'); // Send the output string to MixItUp via the 'filter' method:    
